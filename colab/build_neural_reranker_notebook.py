...
Every cell body below is a module-level raw string, deliberately:

  * **Raw**, so the regexes in the chunking cell keep their backslashes
    without a second layer of escaping.
  * **Not an f-string**, so a brace is always just a brace. The one cell that
    needs a substitution names a placeholder and replaces it. Mixing f-string
    and plain cells in one file is what previously shipped 12 sites reading
    ``{key}`` to the user as literal text -- and a blanket un-escaping pass
    then broke the cells that really were f-strings.
  * **At column zero**, so what is read here is exactly what a reader runs.

This script is not the sole author of the notebook on disk: the validator
writes execution outputs into it. Regenerating preserves those when the code
is unchanged, and drops them when --force changes the code, because an output
that no longer corresponds to its cell is worse than no output at all.
"""
...
#: Replaced with the base64 of the gzipped benchmark. A placeholder rather
#: than an f-string so no other brace in any cell has to be escaped.
BENCHMARK_PLACEHOLDER = "__ENCODED_BENCHMARK__"

#: Metadata key the notebook validator writes its execution record under.
VALIDATION_KEY = "aletheia_validation"
...
#: Dependencies, seeds and working directories.
CODE_SETUP = r"""
...
#: The embedded benchmark. The only cell with a substitution.
CODE_BENCHMARK = r"""
...
ENCODED_BENCHMARK = "__ENCODED_BENCHMARK__"
...
#: Pinned arXiv download, retried, each PDF checked against its hash.
CODE_CORPUS = r"""
...
#: Page-aware chunking. Page numbers are part of the gold labels.
CODE_CHUNKS = r"""
...
#: TF-IDF retrieval and deterministic feature extraction.
CODE_CANDIDATES = r"""
...
#: Leakage-free split by question ID.
CODE_SPLIT = r"""
...
#: The 5-16-8-1 MLP and its training loop.
CODE_TRAIN = r"""
...
#: MRR and Recall@6 against the cosine baseline.
CODE_EVALUATE = r"""
...
#: Save the model, metrics and learning curve.
CODE_ARTIFACTS = r"""
...
def build_notebook() -> dict:
    """The notebook this script owns: cells, in order, with no outputs."""
    ...
    cells = [
        ...
        markdown(MARKDOWN_REPRODUCIBILITY),
        code(CODE_SETUP),
        code(CODE_BENCHMARK.replace(BENCHMARK_PLACEHOLDER, encoded_questions)),
        code(CODE_CORPUS),
        code(CODE_CHUNKS),
...
def _code(notebook: dict) -> list[str]:
    ...
    The embedded benchmark is compared by its decoded content, not its bytes.
    gzip output varies with the zlib build that produced it, so comparing the
    base64 reported drift on a byte no reader of the notebook can observe.
    """
    sources = []
    for cell in notebook["cells"]:
        ...
        match = re.search(r'ENCODED_BENCHMARK = "([^"]*)"', source)
        if match:
            decoded = gzip.decompress(base64.b64decode(match.group(1)))
            canonical = json.dumps(json.loads(decoded), sort_keys=True)
            source = source.replace(match.group(1), canonical)
        sources.append(source)
    return sources


def main(force: bool = False, stdout: bool = False, check: bool = False) -> None:
    notebook = build_notebook()
    existing = json.loads(OUTPUT.read_text(encoding="utf-8")) if OUTPUT.exists() else None
    drifted = existing is not None and _code(existing) != _code(notebook)

    if check:
        if drifted:
            raise SystemExit(
                f"{OUTPUT.name} has drifted from {Path(__file__).name}. "
                "Reconcile them before committing."
            )
        print(f"{OUTPUT.name} matches {Path(__file__).name}.")
        return
    ...
    # outputs and validation record written by validate_research_notebooks.py,
    # and any fix applied to the notebook directly. Overwriting unconditionally
    # is how a regeneration silently reverts them -- it is what would have
    # dropped the pypdf parser, the download retry loop and the make_chunks
    # fix. Compare the code and refuse rather than discard.
    if drifted and not force:
        raise SystemExit(
            f"{OUTPUT.name} on disk differs from what this script generates, "
            "so the notebook carries changes this generator does not. Port "
            "them into this script, then rerun with --force.\n\nSee the "
            "difference with:\n  python colab/build_neural_reranker_notebook.py "
            "--stdout > gen.ipynb\n  git diff --no-index gen.ipynb "
            f"{OUTPUT.relative_to(ROOT).as_posix()}"
        )

    if existing is not None and not drifted:
        # Same code, so the recorded outputs still describe these exact cells
        # and are kept. This is what makes regenerating after a markdown edit
        # safe rather than destructive.
        previous_code = [c for c in existing["cells"] if c["cell_type"] == "code"]
        for cell, previous in zip(
            [c for c in notebook["cells"] if c["cell_type"] == "code"], previous_code
        ):
            cell["outputs"] = previous.get("outputs", [])
            cell["execution_count"] = previous.get("execution_count")
        if VALIDATION_KEY in existing.get("metadata", {}):
            notebook["metadata"][VALIDATION_KEY] = existing["metadata"][VALIDATION_KEY]
        note = "outputs and validation record preserved"
    else:
        note = (
            "outputs dropped -- the code changed, so the recorded run no longer "
            "describes it; rerun colab/validate_research_notebooks.py"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="Exit non-zero if the notebook and this script have diverged.",
    )
